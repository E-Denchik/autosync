...
`127.0.0.1` и принимает запросы только через собственное окно (по
одноразовому токену на процесс) — AutoSync недоступен ни из браузера на
этой машине, ни тем более с других устройств в сети. Закрытие окна
полностью завершает приложение. Открывается сразу на главном экране —
без входа, регистрации и настройки.
...
GitHub Release (см. «Скачать готовую сборку» выше).
...
другого устройства в сети AutoSync недоступен.
