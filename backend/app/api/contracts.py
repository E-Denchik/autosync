...
from app.models import Contract, ContractFile, ContractLaborNorm, ContractPart, DocumentProcessingStatus
...
def create_contract():
    ...
    log_change("contract", contract.id, "created", actor=get_current_user(), details={"name": name})
    db.session.commit()
...
def import_more_files(contract_id: int):
    ...
    log_change(
        "contract", contract.id, "import_added", actor=get_current_user(), details={"file_count": len(files)}
    )
    db.session.commit()
...
def delete_contract(contract_id: int):
    ...
    for path in _contract_paths(contract):
        if os.path.isfile(path):
            os.remove(path)
    log_change("contract", contract.id, "deleted", actor=get_current_user())
