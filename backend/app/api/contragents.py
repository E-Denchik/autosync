...
def list_hourly_rates(contragent_id: int):
    ...
    rates = (
        ...
        .order_by(ContragentHourlyRate.vehicle_make, ContragentHourlyRate.vehicle_model)
        .all()
    )
...
def create_hourly_rate(contragent_id: int):
    ...
    rate = ContragentHourlyRate(
        contragent_id=contragent_id, vehicle_make=vehicle_make, vehicle_model=vehicle_model, hourly_rate=hourly_rate
    )
    db.session.add(rate)
    db.session.commit()
...
def import_hourly_rates_file(contragent_id: int):
    ...
    try:
        result = import_hourly_rates(ContragentHourlyRate, "contragent_id", contragent_id, path, llm_client=llm_client)
