...
- **Rossco** (`rossco_client.py`, SOAP) — цены, наличие, кросс-номера запчастей. Подключён и подтверждён реальными ключами заказчика.
- **АвтоЕвро** (`autoeuro_client.py`, REST) — то же самое. Подключён и подтверждён реальными ключами заказчика.
- **Москворечье** (`moskvorechye_client.py`, протокол ABCP) — то же самое. Подключён и подтверждён реальными ключами заказчика.
- **1С:Альфа-Авто** (OData, HTTP Basic Auth) — номенклатура/остатки (`nomenclature_client.py`) и нормо-часы (`autodata_client.py`) с одного и того же сервера заказчика.

Три поставщика запчастей выше опрашиваются параллельно и объединяются в
один список кросс-номеров (`parts_supplier_client.py: AggregatedPartsSupplierClient`)
для автоматического сопоставления в `matcher.py`; для ручного подбора
оператором те же клиенты используются напрямую через `supplier_search.py`
(см. `SupplierSearchModal.jsx`). Изначально задуманный единый
`PARTS_SUPPLIER_BASE_URL`/`PARTS_SUPPLIER_API_KEY` (generic `PartsSupplierClient`
в том же файле) остаётся как опциональный дополнительный источник — схема
его ответа заказчиком не подтверждена и не используется, если ключи не заданы.
...
│   │   │   ├── parts_supplier_client.py# агрегатор поставщиков запчастей для автосопоставления (см. ниже)
│   │   │   ├── rossco_client.py        # Rossco (SOAP) — цены/наличие/кросс-номера
│   │   │   ├── autoeuro_client.py      # АвтоЕвро (REST) — цены/наличие/кросс-номера
│   │   │   ├── moskvorechye_client.py  # Москворечье (протокол ABCP) — цены/наличие/кросс-номера
│   │   │   ├── supplier_search.py      # ручной поиск по поставщикам запчастей (для UI)
...
- **Confidence-статусы в сопоставлении запчастей и работ свёрнуты до "проверено"/"догадка"** (см. `backend/app/services/confidence_display.py::is_verified`, `ConfidenceBadge.jsx`) — сознательный пересмотр более раннего решения "LLM-догадка никогда не должна выглядеть так же надёжно, как точное совпадение": по решению заказчика гоняться за повышением точности (доп. запросы к API поставщиков/LLM) невыгодно, дешевле дать оператору один быстрый способ доделать позицию самому. Догадка LLM выше порога уверенности (`MATCH_CONFIDENCE_THRESHOLD`) теперь намеренно отображается как "проверено" наравне с exact/cross_ref. Это касается только визуального бейджа уверенности — сам workflow не ослаблен: `review_status` (pending/approved/rejected) — отдельное поле, ручное принятие/отклонение по-прежнему обязательно, и `generate_document` по-прежнему блокирует генерацию документа, пока есть хоть одна PENDING-позиция.
- **Человек в контуре на изменении цен и на low-confidence сопоставлениях** — до накопления статистики точности автоприменение отключено намеренно. Позиции, не попавшие в "проверено" (см. выше), дополнительно получают инлайн-поле для ручной правки прямо в таблице (`InlineCorrection.jsx`), а не только модалку с поиском по каталогу — тот же принцип "человек в контуре", просто с более быстрым способом его закрыть.
...
- **Backend слушает только `127.0.0.1`** (`native_app.py: run_backend`) —
  ...
  в трее.
- Нет учётных записей и входа — приложение открывается сразу на главном
  экране, все разделы доступны без ограничений. Доступ к backend закрыт не
  через логин, а через одноразовый токен на процесс, который окно передаёт
  на каждый запрос (`app/__init__.py: _require_local_session_token`).
